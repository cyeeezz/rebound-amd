...
_PALETTE = {"honeydew": "#F6FFEA", "peach": "#FFDE96", "coral": "#FA855A",
            "tomato": "#C93638", "sky": "#62C4DA"}

# --- lucide-style inline icons (no emoji anywhere in the UI) ----------------
_ICON_PATHS = {
...
def _icon(name, size=18, stroke="currentColor", sw=2):
...
def render_rebound_logo(compact=False):
    """Official Rebound lockup as clean inline SVG (8th reference image)."""
    dark = st.session_state.get("theme") == "dark"
    word = "#F7F8FA" if dark else "#17233A"
    coral = _PALETTE["coral"]
    sym = (f"<svg width='30' height='30' viewBox='0 0 40 40' fill='none' role='img' "
    ...
    if compact:
        return f"<span style='display:inline-flex;align-items:center'>{sym}</span>"
    return (f"<span style='display:inline-flex;align-items:center;gap:8px'>{sym}"
            f"<span style='font-family:Inter,Arial,sans-serif;font-weight:800;"
            f"font-size:1.2rem;color:{word}'>Rebound</span></span>")


def _page_title(title, icon=None):
    ico = (_icon(icon, 26, _PALETTE["coral"], 2.2) + " ") if icon else ""
    st.markdown(f"<h1 style='display:flex;align-items:center;gap:10px'>{ico}{title}</h1>",
                unsafe_allow_html=True)


def _section(title, icon=None):
    ico = (_icon(icon, 20, _PALETTE["coral"], 2.2) + " ") if icon else ""
    st.markdown(f"<h3 style='display:flex;align-items:center;gap:8px'>{ico}{title}</h3>",
                unsafe_allow_html=True)


# --- theme CSS (single source) ---------------------------------------------
def _theme_vars(theme):
    if theme == "dark":
        return {"app_bg": "#080A0F", "surface": "#12161E", "elevated": "#181D27",
                "text": "#F7F8FA", "muted": "#A7AFBE", "border": "#2A303B",
                "dot": "#232A36",
                "glow": ("radial-gradient(900px 500px at 12% -10%, rgba(250,133,90,.10), transparent 60%),"
    ...
    return {"app_bg": "#FFFFFF", "surface": "#FFFFFF", "elevated": "#FFFFFF",
            "text": "#17233A", "muted": "#667085", "border": "#E5E8EF",
            "dot": "#E5E8EF",
            "glow": ("radial-gradient(820px 460px at 8% -8%, rgba(98,196,218,.16), transparent 60%),"
...
_PRIORITY_CHIP = {"High": ("rgba(250,133,90,.16)", "#8a3b1e"),
                  "Medium": ("rgba(255,222,150,.38)", "#7a5a17"),
                  "Low": ("rgba(98,196,218,.18)", "#0d5c6e")}


def _priority_chip(p):
    bg, fg = _PRIORITY_CHIP.get(p, ("rgba(98,196,218,.18)", "#0d5c6e"))
    return _chip(f"{p} priority", bg, fg)


_STATUS_TEXT = {"pending": "Planned", "completed": "Completed", "skipped": "Skipped"}


def _status_chip(state):
    if state == "completed":
        return _chip("Completed", "#F6FFEA", "#3f6b1f")
    if state == "skipped":
        return _chip("Skipped", "rgba(120,120,120,.16)", "#5b5b5b")
    return _chip("Planned", "rgba(98,196,218,.18)", "#0d5c6e")


_TINTS = [("rgba(246,255,234,.7)", "#3f6b1f"), ("rgba(98,196,218,.14)", "#0d5c6e"),
          ("rgba(255,222,150,.30)", "#7a5a17"), ("rgba(250,133,90,.14)", "#8a3b1e"),
          ("rgba(150,140,220,.14)", "#4b3f7a")]


def _topic_tint(topic):
    return _TINTS[hash(topic) % len(_TINTS)]
...
def render_today_session_card(session, status, all_days, expanded=False, keyprefix=""):
    ...
    start = date.today()
    bg, fg = _topic_tint(topic)
    with st.container(border=True):
        head = st.columns([4, 2])
        head[0].markdown(f"<div class='rb-serif' style='font-size:1.15rem;color:{fg}'>{topic}</div>",
                         unsafe_allow_html=True)
        head[1].markdown(f"<div style='text-align:right'>{_priority_chip(session.get('priority','Medium'))} "
                         f"{_status_chip(state)}</div>", unsafe_allow_html=True)
        st.caption(f"{session['duration']} min · {_section_for(topic).get('estimated_difficulty','')}")
        pts = _key_points(topic, 3 if expanded else 2)
        if pts:
            st.write(", ".join(pts))
        show = expanded or st.session_state.get(f"open_{keyprefix}{topic}", False)
        if not expanded:
            if st.button("Open" if not show else "Close", key=f"tog_{keyprefix}{topic}"):
                st.session_state[f"open_{keyprefix}{topic}"] = not show
                st.rerun()
        if show:
            if state == "pending":
                c = st.columns(4)
                if c[0].button("Start", type="primary", key=f"{keyprefix}start_{topic}"):
                    st.session_state["active_session"] = topic
                    st.rerun()
                if c[1].button("Complete", key=f"{keyprefix}done_{topic}"):
                    status[topic] = "completed"
                    st.session_state["session_status"] = status
                    st.rerun()
                with c[2].popover("Move"):
                    ...
                    if st.button("Confirm move", key=f"{keyprefix}mv_btn_{topic}"):
                        ...
                        st.rerun()
                if c[3].button("Skip", key=f"{keyprefix}skip_{topic}"):
...
def render_setup_split_layout():
    ...
    with left:
        ...
        st.markdown(f"<div style='margin:14px 0'>{_chip('Setup your exam', 'rgba(250,133,90,.14)', '#8a3b1e')}</div>",
                    unsafe_allow_html=True)
        st.markdown("<div class='rb-hero'>Turn your notes into<br>a study plan that adapts.</div>",
                    unsafe_allow_html=True)
        st.write("Upload your revision notes and we'll analyse them, then build a personalised "
                 "plan to help you recover and perform your best.")
        f = st.columns(3)
        for col, (t, d) in zip(f, [("Upload notes", "Add your revision notes in PDF format."),
                                   ("We analyse", "Rebound extracts key topics and concepts."),
                                   ("You rebound", "Receive an adaptive plan that changes with you.")]):
            col.markdown(f"<div class='rb-feat'>{t}</div>", unsafe_allow_html=True)
            col.caption(d)
    with right:
        with st.container(border=True):
            _section("Let's set up your exam", "settings")
            st.caption("Tell us about your exam and upload your notes to begin.")
        ...
        if st.session_state.get("sections"):
            ...
            with st.container(border=True):
                st.markdown(f"{_icon('check', 18, '#3f6b1f')} **Analysed {n} topic"
                            f"{'s' if n != 1 else ''} from {st.session_state.get('file_name', 'your document')}.**",
                            unsafe_allow_html=True)
                st.caption("We've identified key concepts and are ready to build your plan.")
                c1, c2 = st.columns(2)
                if c1.button("View Knowledge Map", key="setup_kmap"):
                    _nav_to("Knowledge Map")
                if c2.button("Skip to Diagnostic", type="primary", key="setup_diag"):
                    _nav_to("Diagnostic")
...
def page_home():
    ...
    with main:
        top = st.columns([4, 1])
        with top[0]:
            _section("Today's Study", "calendar")
            st.caption("Your personalised plan for today.")
        top[1].markdown(f"<div style='text-align:right' class='rb-muted'>{len(todays)} sessions today</div>",
                        unsafe_allow_html=True)
        if todays:
            for i, sess in enumerate(todays):
                render_today_session_card(sess, status, all_days, expanded=(i == 0), keyprefix="home_")
        else:
            with st.container(border=True):
                st.write("Nothing scheduled for today. Enjoy the breather, or look ahead in Study Plan.")

    with side:
        exam = st.session_state.get("exam_date")
        with st.container(border=True):
            _section("Exam Countdown", "calendar")
            if exam:
                try:
                    d = max((exam - date.today()).days, 0)
                    st.markdown(f"<div class='rb-serif' style='font-size:2rem'>{d} days</div>",
                                unsafe_allow_html=True)
                ...
                st.caption("Set an exam date in Setup.")
        with st.container(border=True):
            _section("Today's Total Study Time", "clock")
            tmin = sum(s["duration"] for s in todays)
            st.markdown(f"<div class='rb-serif' style='font-size:1.7rem'>{tmin // 60}h {tmin % 60}m</div>",
                        unsafe_allow_html=True)
            st.caption("of focused study")
        with st.container(border=True):
            _section("Progress Summary", "chart")
            prog = plan_progress(plan, status)
            active = 1 if st.session_state.get("active_session") else 0
            planned = prog["total"] - prog["completed"] - prog.get("skipped", 0)
            st.markdown(f"<div class='rb-serif' style='font-size:1.7rem'>{prog['pct']:.0%} complete</div>",
                        unsafe_allow_html=True)
            st.progress(prog["pct"])
            st.write(f"{prog['completed']} completed · {active} in progress · {max(planned, 0)} planned")
        with st.container(border=True):
            _section("Quick Actions", "zap")
            if st.button("Upload another document", key="qa_upload"):
                _nav_to("Setup")
            if st.button("Review availability", key="qa_recovery"):
                _nav_to("Recovery")
